...
public class MemberController {
    ...
    public ResponseEntity<String> findUsername(@Valid @RequestBody MemberFindUsernameRequest request) {
        memberService.findUsername(request);
        return ResponseEntity.ok("아이디가 이메일로 전송되었습니다.");
    }

    @PostMapping("/findPassword")
    public ResponseEntity<String> findPassword(@Valid @RequestBody MemberFindPasswordRequest request) {
        memberService.findPassword(request);
        return ResponseEntity.ok("임시 비밀번호가 이메일로 전송되었습니다.");
    }

    @PostMapping("/changePassword")
    public ResponseEntity<String> changePassword(@Valid @RequestBody MemberChangePasswordRequest request) {
        memberService.changePassword(request);
        return ResponseEntity.ok("비밀번호가 변경되었습니다.");
    }

    @PostMapping("/recharge")
    public ResponseEntity<String> rechargeMoney(@Valid @RequestBody MoneyRechargeRequest request,
